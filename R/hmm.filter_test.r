...
hmm.filter_test <- function(g,L,K1,K2,P){
...
#   pred[2,T,,] <- L[T,,] # last position is known
  ...
  for(t in 2:T){
    ...
    psi[t-1] <- sum(as.vector(post1), na.rm=T) + sum(as.vector(post2), na.rm=T) # this step ensures that the probability sums to one across all states
    ...
    phi[1,t,,] <- post1/(psi[t-1]+1e-15) # this step ensures that the probability sums to one across all states
    phi[2,t,,] <- post2/(psi[t-1]+1e-15) # this step ensures that the probability sums to one across all states
  }
