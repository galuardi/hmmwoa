...
g <- setup.grid(locs, res = 'one') # make sure loading function from misc_funs.r
ngrid <- rev(dim(g$lon))
lon <- g$lon[1,]
lat <- g$lat[,1]
...
L.locs.a <- L.locs.a[,,c(1,which(dateVec %in% as.Date(locs$Date)),T)]

# now need to re-format the array to match dims in sphmm (time, lat, lon)
La.re <- aperm(L.locs.a, c(3,2,1))
# check that it worked ok
image.plot(g$lon,g$lat,La.re[4,,])

## EVENTUALLY WE'LL USE SOMETHING LIKE THIS TO SYNC LIKELIHOOD RASTERS
sync = F
if(sync){
